...
def get_exercises(patient_name):
    c.execute("SELECT * FROM exercises WHERE patient_name=?", (patient_name,))
...
def main():
    if state.current_user != 'admin':
        ...
        log_exercise()
        patient_name = st.text_input("Enter patient name to view exercises")
        if patient_name:
            exercises = get_exercises(patient_name)
